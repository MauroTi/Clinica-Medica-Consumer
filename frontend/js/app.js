...
function getConsultaStatusLabel(status) {
  ...
  const chave = String(status || "agendada")
    .toLowerCase()
    .trim();
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", async (e) => {
      ...
      btn.disabled = true;
      btn.innerHTML =
        '<i class="fas fa-sync-alt me-1 fa-spin"></i> Atualizando...';
...
function updateDashboard() {
  document.getElementById("totalPacientes").textContent = formatNumber(
    state.pacientes.length,
  );
  document.getElementById("totalMedicos").textContent = formatNumber(
    state.medicos.length,
  );
  document.getElementById("totalConsultas").textContent = formatNumber(
    state.consultas.length,
  );
...
function renderPacientesTable() {
  ...
  if (total === 0) {
    tbody.innerHTML =
      '<tr class="text-center"><td colspan="5" class="py-4">Nenhum paciente encontrado</td></tr>';
...
function renderMedicosTable() {
  ...
  if (total === 0) {
    tbody.innerHTML =
      '<tr class="text-center"><td colspan="6" class="py-4">Nenhum médico encontrado</td></tr>';
...
function renderConsultasTable() {
  ...
  if (total === 0) {
    tbody.innerHTML =
      '<tr class="text-center"><td colspan="8" class="py-4">Nenhuma consulta encontrada</td></tr>';
  ...
  if (!consultasSorted.length) {
    ...
    state.consultas.forEach((c) => {
      ...
      if (nomeMedico && nomeMedico !== "Desconhecido") {
        consultasPorMedico[nomeMedico] =
          (consultasPorMedico[nomeMedico] || 0) + 1;
      }
    });

    consultasSorted = Object.entries(consultasPorMedico).sort(
...
function filterTable(tableId, searchTerm) {
  ...
  if (tableId === "tablePacientes") {
    ...
    state.pagination.pacientes.filtered = state.pacientes.filter((p) => {
      const texto =
        `${p.nome || ""} ${p.cpf || ""} ${p.telefone || ""} ${p.email || ""}`.toLowerCase();
  ...
  if (tableId === "tableMedicos") {
    ...
    state.pagination.medicos.filtered = state.medicos.filter((m) => {
      const texto =
        `${m.nome || ""} ${m.crm || ""} ${m.especialidade || ""} ${m.telefone || ""} ${m.email || ""}`.toLowerCase();
      return texto.includes(termo);
    });
    renderMedicosTable();
    return;
  ...
  if (tableId === "tableConsultas") {
    ...
    state.pagination.consultas.filtered = state.consultas.filter((c) => {
      const texto =
        `${getNomePacienteConsulta(c)} ${getNomeMedicoConsulta(c)} ${formatarData(getConsultaData(c))} ${getConsultaHora(c)} ${getEspecialidadeConsulta(c)} ${getConsultaStatusLabel(getConsultaStatus(c))} ${getConsultaDescricao(c)}`.toLowerCase();
      return texto.includes(termo);
    });
...
function applyAllFiltersAndRender() {
  ...
  filterTable(
    "tableMedicos",
    document.getElementById("searchMedicos")?.value || "",
  );
  filterTable(
    "tableConsultas",
    document.getElementById("searchConsultas")?.value || "",
  );
}
...
function updateServerStatus(connected) {
  ...
  if (connected) {
    ...
    status.parentElement.querySelector(".fa-circle").className =
...
function deletarPaciente(id) {
  ...
  fetch(`${API_URL}/pacientes/${id}`, { method: "DELETE" })
    .then(async (res) => {
      ...
      if (!res.ok)
        throw new Error(
          data?.mensagem || data?.message || "Erro ao deletar paciente",
        );
...
function adicionarPaciente() {
  ...
  document.getElementById("modalPacienteTitle").textContent =
    "Adicionar Paciente";
  const modal = new bootstrap.Modal(document.getElementById("modalPaciente"));
  modal.show();
}
...
function salvarPaciente() {
  ...
  fetch(url, {
    ...
    .then(async (res) => {
      ...
      if (!res.ok)
        throw new Error(
          data?.mensagem || data?.message || "Erro ao salvar paciente",
        );
      showSuccess(
        estadoEdicao.paciente
...
function deletarMedico(id) {
  ...
  fetch(`${API_URL}/medicos/${id}`, { method: "DELETE" })
    .then(async (res) => {
      ...
      if (!res.ok)
        throw new Error(
          data?.mensagem || data?.message || "Erro ao deletar médico",
        );
...
function salvarMedico() {
  ...
  fetch(url, {
    ...
    .then(async (res) => {
      ...
      if (!res.ok)
        throw new Error(
          data?.mensagem || data?.message || "Erro ao salvar médico",
        );
      showSuccess(
        estadoEdicao.medico
...
function adicionarConsulta() {
  ...
  document.getElementById("modalConsultaTitle").textContent =
    "Adicionar Consulta";
...
function editarConsulta(id) {
  ...
  document.getElementById("consultaPaciente").value = String(
    consulta.paciente_id || "",
  );
  document.getElementById("consultaMedico").value = String(
    consulta.medico_id || "",
  );
  document.getElementById("consultaData").value = formatarDataInput(
    getConsultaData(consulta),
  );
  document.getElementById("consultaDescricao").value =
    getConsultaDescricao(consulta) === "-"
      ? ""
      : getConsultaDescricao(consulta);
  document.getElementById("consultaStatus").value =
    getConsultaStatus(consulta) || "agendada";
  document.getElementById("consultaObservacaoStatus").value =
    consulta.observacao_status || "";
...
function popularSelectsConsulta() {
  ...
  selectMedico.innerHTML =
    ...
    state.medicos
      .map(
        (m) =>
          `<option value="${m.id}">${m.nome} - ${m.especialidade || "Sem especialidade"}</option>`,
      )
      .join("");
}
...
async function salvarConsulta() {
  ...
  const status = (document.getElementById("consultaStatus").value || "agendada")
    .trim()
    .toLowerCase();
  const observacao_status = document
    .getElementById("consultaObservacaoStatus")
    .value.trim();
  ...
  const url = estadoEdicao.consulta
    ? `${API_URL}/consultas/${estadoEdicao.consulta}`
    : `${API_URL}/consultas`;
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        data?.mensagem || data?.message || "Erro ao salvar consulta",
      );
    }

    showSuccess(
      estadoEdicao.consulta
...
function deletarConsulta(id) {
  ...
  fetch(`${API_URL}/consultas/${id}`, { method: "DELETE" })
    .then(async (res) => {
      ...
      if (!res.ok)
        throw new Error(
          data?.mensagem || data?.message || "Erro ao deletar consulta",
        );
